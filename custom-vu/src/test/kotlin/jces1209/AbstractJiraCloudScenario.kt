...
abstract class AbstractJiraCloudScenario {
    fun shouldRunScenarioWithoutErrors(jira: Jira, user: User, driver: RemoteWebDriver) {
        val scenario = JiraCloudScenario()
        ...
        val firstBackupElement = isElementPresent(driver, By.id("del_10001"))
        val secondBackupElement = isElementPresent(driver, By.id("del_10200"))
    ...
    private fun goToServices(driver: RemoteWebDriver, jira: Jira) {
        ...
        if (isElementPresent(driver, By.id("login-form-authenticatePassword"))) {
    ...
    private fun isElementPresent(
        driver: RemoteWebDriver,
        locator: By
    ): Boolean {
        return driver.findElements(locator).isNotEmpty()
    }
}
